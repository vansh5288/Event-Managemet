...
router.get('/event/:eventId', async (req, res, next) => {
    try {
        const reviews = await Review_1.Review.find({ event: req.params.eventId })
...
router.post('/', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        const existingReview = await Review_1.Review.findOne({ event, user: req.userId });
        if (existingReview) {
        ...
        const reviews = await Review_1.Review.find({ event });
        const avgRating = reviews.reduce((sum, r) => sum + r.rating, 0) / reviews.length;
        await Event_1.Event.findByIdAndUpdate(event, {
...
router.put('/:id', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        await review.save();
        // Update event rating
        const reviews = await Review_1.Review.find({ event: review.event });
        const avgRating = reviews.reduce((sum, r) => sum + r.rating, 0) / reviews.length;
        await Event_1.Event.findByIdAndUpdate(review.event, {
            rating: Math.round(avgRating * 10) / 10,
            reviewCount: reviews.length,
        });

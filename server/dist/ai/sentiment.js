...
exports.detectDuplicateEvents = exports.generateEventSummary = exports.analyzeSentiment = void 0;
 ...
 */
const generateEventSummary = async (event, reviews) => {
    if (!genAI) {
        return {
 ...
 */
const detectDuplicateEvents = async (title, description) => {
    const events = await Event_1.Event.find({ status: { $ne: Event_1.EventStatus.CANCELLED } }).select('title description category');
    const duplicates = events
        .map((event) => {
        const titleSimilarity = calculateSimilarity(title, event.title);
        const descSimilarity = calculateSimilarity(description, event.description || '');
...
exports.detectDuplicateEvents = detectDuplicateEvents;

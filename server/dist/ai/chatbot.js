...
exports.detectSpam = exports.generateEventEmail = exports.getChatbotResponse = void 0;
const generative_ai_1 = require("@google/generative-ai");
const config_1 = require("../config");
let genAI = null;
if (config_1.config.googleAiApiKey) {
    genAI = new generative_ai_1.GoogleGenerativeAI(config_1.config.googleAiApiKey);
}
const systemPrompt = `
 ...
 */
const getChatbotResponse = async (message, userContext) => {
    if (!genAI) {
 ...
 */
const generateEventEmail = async (eventName, emailType, audience, details) => {
    if (!genAI) {
...
exports.generateEventEmail = generateEventEmail;
/**
...
const detectSpam = async (text) => {
    ...
    if (genAI && text.length > 20) {
        try {
            const model = genAI.getGenerativeModel({ model: 'gemini-pro' });
            const result = await model.generateContent(`Is the following message spam? Reply with "yes" or "no" only.\n\n"${text}"`);
            const response = await result.response;
...
const ruleBasedEmail = (eventName, emailType, audience) => {
    ...
    <div style="font-family: 'Inter', sans-serif; max-width: 480px; margin: 0 auto; padding: 32px; background: linear-gradient(135deg, #f0f9ff, #e0f2fe); border-radius: 16px;">
      ...
      <a href="http://localhost:5173" style="display: inline-block; padding: 12px 24px; background: linear-gradient(135deg, #3b82f6, #06b6d4); color: white; text-decoration: none; border-radius: 10px; font-weight: 600;">Learn More</a>
    </div>
  `;
